...
const historyConfig = Facet.define<HistoryConfig, Required<HistoryConfig>>({
  combine(configs) {
    return combineConfig(configs, {
      ...
      minDepth: 100,
      newGroupDelay: 500
    }, {editor:null,uuid:null,minDepth: Math.max, newGroupDelay: Math.min})
  }
})
...
const historyField_ = StateField.define({
  ...
  update(state: HistoryState, tr: Transaction): HistoryState {
    ...
    if (event)
      state = state.addChanges(event, time, userEvent, config)
...
export function myHistory(config: HistoryConfig = {}): Extension {
  return [
    historyField_,
...
class HistEvent {
   ...
   * @param effects 
   * @param mapped 
   * @param startSelection 前选择
   * @param selectionsAfter 后选择
   */
  constructor(
...
/**
 * 添加选择
 * @param branch 分支（undo，redo）
 * @param selection 编辑器选择
 * @returns 
 */
...
class HistoryState {
   ...
   * @param done 完成集合
   * @param undone 
   * @param prevTime 前置事件的时间
   * @param prevUserEvent 前置事件
   */
  ...
  addChanges(event: HistEvent, time: number, userEvent: string | undefined, config:Required<HistoryConfig>): HistoryState {
...
export const myHistoryKeymap: readonly KeyBinding[] = [
