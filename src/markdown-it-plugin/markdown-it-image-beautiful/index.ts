...
function image(tokens: Token[], idx: number, options: Object, env: Object, slf: Renderer) {
  ...
  if (src) {
    if (Reg.imgBase64Reg.test(src)) decodeSrc = src
      ...
      //decodeSrc = Reg.windowAbsFilePathReg.test(dec) || Reg.linuxAbsFilePathReg.test(dec)?dec:""
    }
  }
  ...
  //相对路径
  if (decodeSrc && Reg.relativeFilePathReg.test(decodeSrc)) {
    editor.ir.applicationEventPublisher.publish("GET-CurrFilePath")
    token.attrPush(["data-map", decodeSrc])
    const currdir = Reg.dirPathReg.exec(currFilePath).at(0)
    token.attrSet("src",path.join(currdir, decodeSrc))
  }
  ...
  let text = toImgElementText(token.content, decodeSrc ? decodeSrc : src)
  span.innerHTML = text
